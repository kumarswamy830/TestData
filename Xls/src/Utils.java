package com.disney.selenium.common;
...
import com.disney.selenium.common.enums.WdprUrl;
import com.disney.selenium.common.exception.DomNotCompleteException;
import com.disney.selenium.common.exception.DomNotInteractiveException;
import com.disney.selenium.common.exception.UnexpectedPageUrlException;
import com.disney.selenium.common.exception.UrlNotLoadedException;
import com.disney.selenium.common.exception.automation.WebElementNotDisappearedException;
import com.disney.selenium.common.exception.automation.WebElementNotFoundException;
import com.disney.selenium.common.exception.automation.WebElementNotVisibleException;
import com.disney.selenium.common.exception.uimessage.WebUiBlankPageException;
import com.disney.selenium.common.exception.uimessage.WebUiErrorException;
...
public class Utils {
     ...
     * 
     * @param driver
     * @return The Utils instance associated with the driver or null if it does not exist.
     */
     ...
     * 
     * @param conversationId The unique string (GUID) for the session.
     */
    ...
    /**
     * It's common for PEP pages to return infamous "white" pages that 
     * basically do not contain ANY data in it, particularly when 
     * system-dependencies are down.  This will detect if the browser
     * is displaying a "blank/white" page.
     * 
     * @param driver
     * @throws Exception
     */
    ...
    public static boolean checkForBlankPage(WebDriver driver, boolean raiseException) throws Exception {
        ...
        if(result instanceof Boolean) {
            ...
            Log.log(driver).warning("JavaScript result to check for " +
    ...
    public static void waitForDomComplete(WebDriver driver, String errorMsg) throws Exception {
        ExpectedCondition<Boolean> condition = new ExpectedCondition<Boolean>() {
            ...
            public Boolean apply(WebDriver driver) {
                ...
                if(obj == null) {
                    ...
                    Log.log(driver).warning("JavascriptExecutor object is an UNEXPECTED type " 
                            + "[" + obj.getClass().toString() 
                            + "] [" + obj.toString() + "]");
                    return obj.equals("true");
                }
                
                Log.log(driver).warning("JavascriptExecutor object is NOT recognized " 
                        + "[" + obj.getClass().toString() 
                        + "] [" + obj.toString() + "]");
        ...
        try {
            ...
            Log.log(driver).warning("Dom ready-state check is taking a " 
            ...
            try {
                condition = new ExpectedCondition<Boolean>() {
                    ...
                    public Boolean apply(WebDriver driver) {
                        ...
                        if(obj == null) {
                            ...
                            Log.log(driver).warning("Failed waiting for DOM " 
                                    + "[" + obj.getClass().toString() 
                                    + "] [" + obj.toString() + "]");
                        ...
                        return (obj.equals("complete") || obj.equals("interactive"));
                    }
                };
                ...
                String msg = (errorMsg == null || errorMsg.isEmpty()) ?
                             "Failed to wait for DOM readyState" : 
     ...
     */
    private static boolean waitForDomInteractiveNoException(WebDriver driver) throws Exception {
    ...
    public static void waitForDomInteractive(WebDriver driver, String warningMsg) throws Exception {
        ...
        try {
            ...
            String msg = (warningMsg == null || warningMsg.isEmpty()) ?
                         "Failed to wait for DOM readyState" : 
    ...
    /**
     * PLEASE prefer the Enumeration version over this.
     ...
     * @param url The expected URL that the driver should match
     * @param driver
     * @throws Exception
     */
    ...
    public static void waitForUrlLoaded(WebDriver driver, String url, long timeout) throws Exception {
        ...
        ExpectedCondition<Boolean> condition = new ExpectedCondition<Boolean>() {
            ...
            public Boolean apply(WebDriver driver) {
                ...
                driverUrl = (index > -1) ? driverUrl.substring(0, index) : driverUrl;
        ...
        try {
            ...
            throw new UrlNotLoadedException(
                    "Failed to verify driver URL: " + url + " vs " + driver.getCurrentUrl(), 
                    ex, driver);
        }
    }
    
    public static boolean waitForUrlLoadedNoException(WebDriver driver, WdprUrl url) {
        return waitForUrlLoadedNoException(driver, url, Constants.GLOBAL_DRIVER_TIMEOUT);
    }
    
    public static boolean waitForUrlLoadedNoException(WebDriver driver, WdprUrl url, long timeout) {
        boolean isSuccessful = false;
        try {
     ...
     * @throws Exception
     */
    public static void waitForUrlLoaded(WebDriver driver, WdprUrl url) throws Exception {
        waitForUrlLoaded(driver, url, Constants.GLOBAL_DRIVER_TIMEOUT);
    }
    
    public static void waitForUrlLoaded(WebDriver driver, WdprUrl url, long timeout) throws Exception {
        if(!url.isValid()) {
        ...
        try {
            ...
            throw new UrlNotLoadedException("Failed to verify driver URL with " + url.toString() + 
    ...
    /**
     * PLEASE prefer the Enumeration version over this.
     * A strict version of {@link #waitForUrlLoaded(WebDriver, String)} because
     ...
     * @throws Exception
     */
    public static void waitForUrlMatch(WebDriver driver, String url) throws Exception {
        if(url.isEmpty()) {
            throw new UrlNotLoadedException("Url is empty", driver);
        }
     ...
     * @throws Exception
     */
    public static void waitForUrlMatch(WebDriver driver, WdprUrl url) throws Exception {
        if(!url.isValid()) {
            throw new UrlNotLoadedException(url.toString(), driver);
        }
     ...
     * 
     * @param url The expected URL after action
     * @param driver
     * @throws Exception
     */
    public static void waitForUrlContains(WebDriver driver, String urlSegment) throws Exception {
        ...
        try {
            ...
            throw new UrlNotLoadedException(
                    "Failed to verify driver URL: " + urlSegment + " vs " + driver.getCurrentUrl(), 
                    ex, driver);
        }
    }
    
    public static void waitForUrlContains(WebDriver driver, WdprUrl url) throws Exception {
        final String urlStr = url.getUrl();
        ExpectedCondition<Boolean> condition = new ExpectedCondition<Boolean>() {
            @Override
            public Boolean apply(WebDriver driver) {
    ...
    /**
     * A major side-effect of scrolling to an element trigger in a change in
     * the DOM and then will void the internal cache of the Web Element.
     * 
    ...
    public static Actions moveToAndClick(WebDriver driver, WebElement element) throws Exception {
        ...
        try {
            ...
            if(ex.getMessage() != null && ex.getMessage().contains("dead object")) {
    ...
    /**
     * Sends keys but most importantly will also over-write existing any text
     * in the element.
     * 
     * @param driver
     * @param element
     ...
     * @throws Exception
     */
    public static Actions clearAndSendKeys(WebDriver driver, WebElement element, CharSequence keysToSend) throws Exception {
        scrollToElement(driver, element);
        Actions actions = new Actions(driver);
        element.clear();
        actions.moveToElement(element)
    ...
    public static WebElement waitForElement(WebDriver driver, By locator, long timeout) throws Exception {
        ...
        try {
            ...
            if(isLogWebElements()) Log.log(driver).info("Found: " + locator.toString());
        } catch(Exception ex) {
    ...
    public static WebElement waitForVisibleElement(WebDriver driver, By locator, long timeout) throws Exception {
        waitForDomInteractiveNoException(driver);
        WebDriverWait wait = new WebDriverWait(driver, timeout);
        ...
        try {
            element = wait.until(condition);
            if(Utils.logWebElements) Log.log(driver).info("Found: " + locator.toString());
        } catch(Exception ex) {
    ...
    public static WebElement waitForElement(WebDriver driver, WebElement parent, By locator, long timeout) throws Exception {
        waitForDomInteractiveNoException(driver);
        ...
        try{
            element = wait.until(condition);
            if(Utils.logWebElements) Log.log(driver).info("Found: " + location.toString());
        } catch(Exception ex) {
            throw new WebElementNotFoundException(
                    "Failed to find element: " + location.toString(), 
                    ex, driver);
        }
        return element;
    }
    
    public static void waitForElementExtinct(WebDriver driver, By locator, long timeout) throws Exception {
        waitForDomInteractiveNoException(driver);
        final By location = locator;
        ExpectedCondition<Boolean> condition = new ExpectedCondition<Boolean>() {
            @Override
        ...
        try {
            wait.until(condition);
            if(Utils.logWebElements) Log.log(driver).info("Extinct: " + location.toString());
        } catch(Exception ex) {
            throw new WebElementNotDisappearedException(
                    "Failed to wait for element extinction: " + location.toString(), 
                    ex, driver);
        }
    }
    
    public static void waitForElementDisappear(WebDriver driver, By locator, long timeout) throws Exception {
        waitForDomInteractiveNoException(driver);
        final By location = locator;
        ...
        try {
            wait.until(condition);
            if(Utils.logWebElements) Log.log(driver).info("Disappeared: " + location.toString());
        } catch(Exception ex) {
            throw new WebElementNotDisappearedException(
                    "Failed to wait for element disappearance: " + location.toString(), 
                    ex, driver);
        }
    }
    ...
    public static List<WebElement> waitForElements(WebDriver driver, By locator, long timeout) throws Exception {
        waitForDomInteractiveNoException(driver);
        WebDriverWait wait = new WebDriverWait(driver, timeout);
        ...
        try {
            ...
            throw new WebElementNotFoundException(locator.toString(), ex, driver);
        }
        return list;
    }
    
    public static List<WebElement> waitForAnyElements(WebDriver driver, By locator, long timeout) throws Exception {
        waitForDomInteractiveNoException(driver);
        WebDriverWait wait = new WebDriverWait(driver, timeout);
        ExpectedCondition<List<WebElement>> condition = ExpectedConditions.presenceOfAllElementsLocatedBy(locator);
    ...
    public static List<WebElement> waitForAnyElements(WebDriver driver, WebElement parent, By locator, long timeout) throws Exception {
        waitForDomInteractiveNoException(driver);
        WebDriverWait wait = new WebDriverWait(driver, timeout);
        final By location = locator;
        final WebElement parentElement = parent;
    ...
    public static List<WebElement> waitForVisibleElements(WebDriver driver, By locator, long timeout) throws Exception {
        waitForDomInteractiveNoException(driver);
        final By location = locator;
        WebDriverWait wait = new WebDriverWait(driver, timeout);
        ExpectedCondition<Boolean> condition = new ExpectedCondition<Boolean>() {
        ...
        try {
            ...
            throw new WebElementNotVisibleException(location.toString(), ex, driver);
        }
    ...
    /**
     * Waits for a visible element - Helps avoid running into issues when 
     * trying to click on a web element.
     * 
     * @param driver
     * @param element
     * @return
    ...
    public static boolean waitForVisibleElement(WebDriver driver, WebElement element) throws Exception {
        ...
        try {
            wait.until(condition);
            isSuccessful = true;
            if(Utils.logWebElements) Log.log(driver).info("Displayed: " + element.toString());
        } catch(Exception ex) {
            throw new WebElementNotVisibleException(
                    "Failed to wait for element visibility: " + webElement.toString(), 
                    ex, driver);
        }
        return isSuccessful;
    }
    ...
    public static <T extends Exception> Exception repackageException(Exception origException, Class<T> newException) {
        ...
        try {
            ...
            for(int i = 0; i < list.length; ++i) {
                ...
                if(origException.getCause() != null 
                    ...
                    T exception = (T) construct.newInstance(origException.getMessage(), origException.getCause());
                    exception.setStackTrace(origException.getStackTrace());
                    return exception;

                } else if(params.length == 1 && params[0].isAssignableFrom(String.class)) {
            ...
            if(constructor != null) {
                T exception = (T) constructor.newInstance(origException.getMessage());
                exception.setStackTrace(origException.getStackTrace());
                return exception;
            }
    ...
    public static <T extends Exception> Exception repackageException(WebDriver driver, T ex, String addition) {
        return repackageException(ex, addition + " " + generateExceptionMsg(driver));
    }
     ...
     * @return
     */
    public static <T extends Exception> Exception repackageException(WebDriver driver, T ex) {
     ...
     */
    @SuppressWarnings("unchecked")
    public static <T extends Exception> Exception repackageException(T ex, String addition) {
    ...
    /**
     * A quick generic screen capture.  The image will be compressed
     * and the image description is defaulted to "Snapshot".
     * 
     * @param driver
     * @return
     */
    ...
    /**
     * Takes a screenshot of driver/browser - appends useful info like 
     * timestamp and test name.  By default this will compress the
     * screen shot to conserve disk space.
     * 
     * @see #captureScreenshot(WebDriver, String, boolean)
     * @param driver
     * @param imageDescription The description of the snapshot - this will be 
     * a part of the PNG file name.
     * @return
     */
            ...
            try {
                ...
                if(shouldCompress) {
                    ...
                    
                    {
                        ...
                        BufferedImage bdest = new BufferedImage((int)(img.getWidth() * 0.66), 
                    ...
                    PngImage image = new PngImage(destinationPng.getAbsolutePath());
    ...
    public static void takeScreenShot(WebDriver driver,String Code,
			String page) {
		...
		createDir(driver,name, "Project Name");

		name = name + "\\GateChanges_Execution_" + scrShotDir;

		createDir(driver,name, "Project_TimeStamp");

		name = name + "\\ProductCode_" +scrShot.format(new Date())+ Code;

		createDir(driver,name, "Product Code");
		...
		strScreenshotName = " Screen Shot Created: " + strScreenshotName;
		
		Log.log(driver).info(strScreenshotName);
	}

	public static void createDir(WebDriver driver,String dirName, String Message) {
		File f = new File(dirName);
		try {
			if (!f.exists()) {
				f.mkdir();
				Log.log(driver).info("Directory Created :: " + Message);
			}
		} catch (Throwable e) {
			Log.log(driver).info("Unable to create directory with " + Message);
		}
	}
    
    /**
     * Handles the exception by printing out the stack trace, driver 
     * information, conversation ID, the current page source i.e. HTML and 
     * even a snapshot to the default log directory.  Verbose information 
     * is sent directly to the error log file.  A less verbose version of 
     * the error is sent to the log file.
     * 
     ...
     * @param printStackTrace Builds stack traces into the error log.
     * @param printCauseToLog Builds all of the errors messages from caused-by exceptions.
     */
    public static void handleException(WebDriver driver, Exception ex, boolean createSnapshot, boolean printStackTrace, boolean printCauseToLog) {
        ...
        if(printStackTrace) {
            ...
            while(cause != null) {
                ...
                errorMsg += Constants.NEW_LINE + "Caused by: " + cause.getMessage() + Constants.NEW_LINE;
                for (int i = 0; i < stackTrace.length; i++) {
                    errorMsg += "\t" + stackTrace[i].toString() + Constants.NEW_LINE;
                }
                ...
                if(cause.getMessage() != null) {
                    if(printCauseToLog) {
                        logMsg += Constants.NEW_LINE + "\tCaused by: " + cause.getMessage();
                    }
                }
        ...
        try {
            if(createSnapshot) {
                String imageDescription = (ex.getClass().getSimpleName() == null ? "Error" : ex.getClass().getSimpleName());
                imageFilename = captureScreenshot(driver, imageDescription, false);
                snapshot = "Screenshot of error: " + imageFilename + Constants.NEW_LINE;
            }
            
        } catch(Exception exception) {
            snapshot = "WARNING, Failed to capure screenshot [" + exception.getMessage() + "]";
            Log.log(driver).warning("Caught exception: " + exception.getMessage());
        ...
        errorMsg = "(" + (driver == null ? "NULL DRIVER" : driver.toString()) + ") " + 
                "Error Location: " + currentUrl + " [[ConvoId:" + conversationId + "]]"
        ...
        if(printStackTrace) {
            ...
            if(log != null) {
                // We want to avoid printing out to stream if it's already going to 
                // console by the regular call with the loggers.
                if(!log.isUsingStdStreamOut()) {
                    ex.printStackTrace();
                }
            } else {
                // If log is null, then lets be on the safe side and print it out.
                ex.printStackTrace();
            }
        }
        ...
        try {
            ...
            if(logPageSource && 
                    !(ex instanceof WebUiErrorException) && 
                ...
                if(!imageFilename.isEmpty()) {
                    ...
                    Log.errorLog(driver).info(Constants.NEW_LINE + driver.getPageSource() + Constants.NEW_LINE);
                }
            }
        ...
        if(driver != null) {
            ...
            if(utils != null) {
                ...
                if(conversationId != null && !conversationId.isEmpty()) {
                    msg = msg.concat(" [[ConvoId:").concat(conversationId).concat("]]");
                }
            }
        }
        return msg;
